...
return{
	get_all_asset: function(req, res){
	...
	add_asset: function(req, res){
		console.log("submit recording of a asset catch: ");
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    // recordAsset - requires 5 args, ID, cost, location, item, holder - ex: args: ['10', 'Hound', '-12.021, 28.012', '1504054225', 'Hansel'], 
		    // send proposal to endorser
		    const request = {
		        //targets : --- letting this default to the peers assigned to the channel
		        chaincodeId: 'asset-app',
		        fcn: 'recordAsset',
		        args: [key, item, location, cost, holder],
		        chainId: 'mychannel',
		        txId: tx_id
		    };
	...
	get_asset: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
			...
			// queryAsset - requires 1 argument
		    const request = {
		        chaincodeId: 'asset-app',
		        txId: tx_id,
		        fcn: 'queryAsset',
	...
	change_holder: function(req, res){
		console.log("changing holder of asset catch: ");
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    if (isProposalGood) {
		        ...
		        console.error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
		        res.send("Error: no asset catch found");
		        // throw new Error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
		    }
		}).then((results) => {
		    console.log('Send transaction promise and event listener promise have completed');
